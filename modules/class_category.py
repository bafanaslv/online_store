...
class Category:
    ...
    def add_product(self, dict_new_product):
        ...
        if found_product is None:
            ...
            new_pr = Product.new_product(dict_new_product)
            ...
            if dict_new_product["price"] <= 0:
                print(f'Введена неверная цена: {dict_new_product["price"]}')
            else:
                if dict_new_product["price"] != found_product.price:
                    if dict_new_product["price"] < found_product.price:
                        if input('Вы согласны на понижение цены товара ?\n') == 'n':
                            return
                    found_product.price = dict_new_product["price"]
